...
export default function AdminDashboard({ onBack }: AdminDashboardProps) {
  const { user, getAllUsers, fetchAllUsers, updateUserRole, updateUserProfile, toggleUserActive, deleteUser, isAdmin } = useStore();
  const [searchQuery, setSearchQuery] = useState('');
  const [roleFilter, setRoleFilter] = useState<UserRole | 'all'>('all');
  const [showDeleteConfirm, setShowDeleteConfirm] = useState<string | null>(null);
  const [editingUser, setEditingUser] = useState<User | null>(null);
  const [editData, setEditData] = useState<EditUserData>({ firstName: '', lastName: '', phone: '', stars: 0 });
            ...
            >
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-4 py-2.5 rounded-lg border border-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 text-sm"
                  />
                  <p className="text-xs text-slate-400 mt-1">Tier updates automatically based on stars</p>
                </div>
              </div>
              ...
              <div className="flex gap-3 mt-6">
                ...
                <button
                  ...
                  disabled={isSaving}
                  className="flex-1 px-4 py-2.5 bg-indigo-600 text-white rounded-lg font-medium hover:bg-indigo-700 transition-colors flex items-center justify-center gap-2 disabled:opacity-70"
                >
                  {isSaving ? (
